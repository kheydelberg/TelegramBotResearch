...
async def admin_panel(call: CallbackQuery, state: FSMContext):
    ...
    if (call.data.endswith('help_admin')):
       await call.message.answer(f'{call.message.from_user.first_name}, помочь пока не могу')
    if (call.data.endswith('show_db')):
        await call.message.answer(f'{call.message.from_user.first_name}, Введите количество строк, которые хотите посмотреть')
        await state.set_state(StepsForm.GET_NUMBER_DB)
    if (call.data.endswith('add_material')):
        await call.message.answer(f'{call.message.from_user.first_name}, начинаем добавлять материал. Введи категорию материала')
    ...
    if (call.data.endswith('delete_material_id')):
        ...
        await call.message.answer(f'{call.message.from_user.first_name}, начинаем удалять материал. Введите id материала, который хотите удалить')
        await state.set_state(StepsForm.GET_ID)
    if (call.data.endswith('show_feedbacks')):
        await call.message.answer(f'{call.message.from_user.first_name}, Введите количество фидбеков, которые хотите посмотреть')
        await state.set_state(StepsForm.GET_NUMBER_FB)
    if (call.data.endswith('show_raw_feedbacks')):
        await call.message.answer(f'{call.message.from_user.first_name}, Введите количество необработанных фидбеков, которые хотите посмотреть')
    ...
    if (call.data.endswith('changing_feedback_status')):
        ...
        await call.message.answer(f'{call.message.from_user.first_name}, начинаем изменение статуса фидбека. Введите id фидбека')
    ...
    if (call.data.endswith('make_backup')):
        ...
        await call.message.answer(f'{call.message.from_user.first_name}, Бэкап тип сделан') 
    if (call.data.endswith('load_backup')):
       # функция, которая загружает бэкап, подробностей пока нет))
       await call.message.answer(f'{call.message.from_user.first_name}, Бэкап тип загружен')
